...
def list_cars():
    ...
    availability = database.get_car_availability(car)
    if(val is None):
        val = []
        flash("Error, car \'{}\' does not exist".format(car))
        page['bar'] = False

    return render_template('car_detail.html', car=val, session=session, availability=availability)
...
def list_bays():
    ...
    if (request.method == 'GET'):
        ...
        val = None
        if(bay == '' or bay == 'None - Click here to add Homebay'):
...
def homebay():
    ...
    if(action == 'update'):
        ...
        # Is it successful?
        if(outcome):
            user_details['homebay'] = database.update_session_homebay(user_details['email'])[0]
            page['bar'] = True
...
def new_booking():
    ...
    if(success == True):
        user_details['num_bookings'] = database.update_session_stat_nrofbookings(user_details['email'])[0]
        page['bar'] = True
        flash("Booking Successful!")
        val = database.get_booking(request.form['book_date'], request.form['book_hour'], request.form['car_regno'])
        ...
        page['bar'] = False
        flash("Car is already booked by another member, or you already have a booking, at this time")
        return(redirect(url_for('new_booking')))
    else:
        page['bar'] = False
        flash("Invalid time. Please select a time greater than the current time")
        return(redirect(url_for('new_booking')))
